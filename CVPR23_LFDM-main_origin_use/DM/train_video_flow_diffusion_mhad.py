...
def main():
    ...
    if args.fine_tune:
        ...
        if os.path.isfile(args.restore_from):
            ...
            for name, param in model_ckpt.items():
                if name in checkpoint['diffusion'] and param.shape == checkpoint['diffusion'][name].shape:
                    model_ckpt[name].copy_(checkpoint['diffusion'][name])
            model.diffusion.load_state_dict(model_ckpt, strict=False)
            print("=> loaded checkpoint '{}'".format(args.restore_from))
            # if "optimizer_diff" in checkpoint.keys():
            #     optimizer_ckpt = model.optimizer_diff.state_dict()
            #     checkpoint_opt = checkpoint['optimizer_diff']
            #     # Ensure the number of parameter groups matches
            #     if len(optimizer_ckpt['param_groups']) == len(checkpoint_opt['param_groups']):
            #         optimizer_ckpt.update(checkpoint_opt)
            #         model.optimizer_diff.load_state_dict(optimizer_ckpt)
        else:
    ...
    while actual_step < args.final_step:
        ...
        for i_iter, batch in enumerate(trainloader):
            ...
            # (batch_size, channel_number, frame_number, height, width)
