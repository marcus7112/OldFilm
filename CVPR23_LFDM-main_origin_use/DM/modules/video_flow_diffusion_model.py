...
class FlowDiffusion(nn.Module):
    ...
    def forward(self):
        ...
        with torch.no_grad():
            ...
            for idx in range(nf):
                ...
                print('______________flow_iteration: ', idx)
        ...
        print('real_vid_grid: ', self.real_vid_grid.shape)
        self.real_vid_conf = torch.stack(real_conf_list, dim=2)
        print('real_vid_conf: ', self.real_vid_conf.shape)
    ...
    def set_train_input(self, ref_img, real_vid, ref_text):
        ...
        if isinstance(ref_text, torch.Tensor):
            self.ref_text = self.generator.compute_fea(ref_text.cuda())
        else:
            self.ref_text = ref_text.cuda()
        print('train input cond dim: ', self.ref_text.shape)

    def set_sample_input(self, sample_img, sample_text):
        self.sample_img = sample_img.cuda()
        if isinstance(sample_text, torch.Tensor):

            self.sample_text = self.generator.compute_fea(sample_text.cuda())
            # self.sample_text=self.diffusion.driving_img_emb(sample_text)
        else:
            self.sample_text = sample_text.cuda()
        print('sample input cond dim: ', self.sample_text.shape)
...
if __name__ == "__main__":
    ...
    # 假设num_frames至少为2
    source_imgs = real_vid[:, :, 0, :, :].clone().detach()  # 所有除最后一帧之外的帧
    print(source_imgs.shape)
    driving_imgs = real_vid[:, :, 1, :, :].clone().detach()  # 所有除第一帧之外的帧

    model = FlowDiffusion(use_residual_flow=False,
                          sampling_timesteps=10,
                          img_size=32,
                          config_pth="/data/223040029/CVPR23_LFDM-main_origin_use/config/mhad128.yaml",
                          pretrained_pth="")
    model.cuda()
    model.train()
    model.set_train_input(ref_img=source_imgs, real_vid=real_vid, ref_text=driving_imgs)
    model.optimize_parameters()
    print("fake_vid_grid:", model.fake_vid_grid.shape)
    print("fake_vid_conf:", model.fake_vid_conf.shape)
    # model.eval()
    # model.set_sample_input(sample_img=source_imgs, sample_text=driving_imgs)
    # model.sample_one_video(cond_scale=1.0)
    # print(model.sample_out_vid.shape)
    # print("sample_vid_conf:", model.sample_vid_conf.shape)
    # print("sample_vid_grid:", model.sample_vid_grid.shape)
