...
# 增加将三维特征图转为一维特征向量的网络 (b,in_channels, 32, 32)->(b,out_channels)
class FeatureEmbedder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv3 = nn.Conv2d(in_channels * 4, out_channels, kernel_size=3, stride=1, padding=1)  # 将输出通道数设置为768
...
class Unet3D(nn.Module):
    ...
    # 当有条件（self.has_cond为True）且cond_scale不等于1时，output是考虑了条件的影响的结果。cond_scale的值决定了条件的影响程度，cond_scale越大，条件的影响越大。
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.driving_img_emb = FeatureEmbedder(256, 768)
    ...
    def p_mean_variance(self, x, t, fea, clip_denoised: bool, cond=None, cond_scale=1.):
        ...
        print('x_recon: ',torch.cat([x, fea,self.driving_fea], dim=1).shape)
        x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn.forward_with_cond_scale(
            torch.cat([x, fea,self.driving_fea], dim=1),
            t,
            cond=cond,
    ...
    def sample(self, fea, cond=None, cond_scale=1., batch_size=16):
        ...
        driving_fea = cond.clone().to(device)
        self.driving_fea = driving_fea.unsqueeze(dim=2).repeat(1, 1, num_frames, 1, 1)

        if is_list_str(cond):
            cond = bert_embed(tokenize(cond), return_cls_repr=self.text_use_bert_cls).to(device)
        elif isinstance(cond, torch.Tensor):
            cond = self.driving_img_emb(cond).to(device)
            print('emb cond: ', cond.shape)
    ...
    def ddim_sample(self, fea, shape, cond=None, cond_scale=1., clip_denoised=True):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            print('x_recon: ', torch.cat([img, fea,self.driving_fea], dim=1).shape)
            pred_noise = self.denoise_fn.forward_with_cond_scale(
                torch.cat([img, fea,self.driving_fea], dim=1),
                time_cond,
                cond=cond,
                cond_scale=cond_scale)
    ...
    def p_losses(self, x_start, t, fea, cond=None, noise=None, clip_denoised=True, **kwargs):
        ...
        print('ploss input cond:', cond.shape)
        driving_fea = cond.clone().to(device)
        driving_fea=driving_fea.unsqueeze(dim=2).repeat(1, 1, f, 1, 1)
        ...
        if is_list_str(cond):
            ...
            none_cond_mask = [ii == "None" for ii in cond]
            cond = self.driving_img_emb(cond)
            cond = cond.to(device)
            print('ploss output cond:', cond.shape)
        print('xt的形状: ', torch.cat([x_noisy, fea, driving_fea], dim=1).shape)
        pred_noise = self.denoise_fn.forward(torch.cat([x_noisy, fea,driving_fea], dim=1), t, cond=cond,
